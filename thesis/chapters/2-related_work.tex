...
Each test consists of a set of $L$ individual presentations, each corresponding to the application of test condition $j$ to image or sequence $k$, potentially repeated $r$ times. For each presentation, subject $i$ provides a discrete score $u_{ijkr}$, forming a matrix of evaluations of dimension $N \times L$, where $N$ is the number of observers. The Mean Opinion Score (MOS) for a given presentation is defined as:

\begin{equation}
\text{MOS}_{jkr} = \frac{1}{N} \sum_{i=1}^{N} u_{ijkr}
\end{equation}

Global MOS values can be obtained by averaging across repetitions or test instances. In double-stimulus protocols, the Difference Mean Opinion Score (DMOS) is computed as:
