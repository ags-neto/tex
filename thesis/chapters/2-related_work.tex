...
Choosing an assessment methodology isn't as straightforward as ticking checkboxes on a list and going with the method that fits the best, we need to account also for what we want to calculate, this is MOS or DMOS, and also, one of the most important aspects, observer's assessibility, if we deploy a complex method this might confuse or throw off the observer, which damages the results.

Many datasets have established a non-written consensus to go with SS methods, these are the most simple, less time consuming, and more versatile methodology, allowing for tests to avoid predefined presentations, or analysis that might tire/annoy observers.

Each test consists of a set of $L$ individual presentations, each corresponding to the application of test condition $j$ to image or sequence $k$, potentially repeated $r$ times. For each presentation, subject $i$ provides a discrete score $u_{ijkr}$, forming a matrix of evaluations of dimension $N \times L$, where $N$ is the number of observers. The \acs{MOS} for a given presentation is defined as:
