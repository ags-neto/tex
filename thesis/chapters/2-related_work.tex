...
Subjective image quality assessment protocols are divided into two types: quality assessment, which captures the overall perceived quality of an image, and impairment assessment, which evaluates the degradation relative to a reference. These are implemented using two principal methodological paradigms: the Single Stimulus (SS) method, where each image is rated independently, and the Double Stimulus (DS) method, where two images are shown and rated comparatively.

Rating scales may be numerical or categorical. Numerical scales range from 1 to 5, 1 to 11, or 0 to 100, and are used to express either quality (e.g., excellent to bad) or impairment (e.g., imperceptible to very annoying). Categorical scales use labeled intervals and are commonly found in ITU protocols. The choice of scale depends on the desired sensitivity and interpretability of the results.

Choosing an assessment methodology isn't as straightforward as ticking checkboxes on a list and going with the method that fits the best, we need to account also for what we want to calculate, \acs{MOS} or \acs{DMOS}, and also, one of the most important aspects, observer's assessibility, if we deploy a complex method this might confuse or throw off the observer, which damages the results.

Many datasets have established a non-written consensus to go with SS methods, these are the most simple, less time consuming, and more versatile methodology, allowing for tests to avoid predefined presentations that might tire and or annoy observers.

Each test session consists of a set of $L$ individual presentations, each corresponding to the application of test condition $j$ to image or sequence $k$, potentially repeated $r$ times. For each presentation, subject $i$ provides a discrete score $u_{ijkr}$, forming a matrix of evaluations of dimension $N \times L$, where $N$ is the number of observers. The \acs{MOS} for a given presentation is defined as:

\begin{equation}
\text{MOS}_{jkr} = \frac{1}{N} \sum_{i=1}^{N} u_{ijkr}
\end{equation}

Global \acs{MOS} values can be obtained by averaging across repetitions or test instances. In double-stimulus protocols, the \acs{DMOS} is computed as:

\begin{equation}
\text{DMOS}_{jkr} = \text{MOS}_{\text{ref},jkr} - \text{MOS}_{\text{dist},jkr}
\end{equation}

To quantify uncertainty in MOS or DMOS, a 95\% confidence interval is computed using the sample standard deviation:
...
This interval assumes a Gaussian distribution of scores and indicates that the true population mean lies within the range with 95\% probability. When averaging across multiple images, caution is advised as inter-image variability can dominate the standard deviation in small datasets.

If the raw scores have been transformed, e.g., through boundary compensation or conversion to impairment response functions, the confidence intervals must be recomputed using transformed variables. In this case, the standard deviation becomes a function of each point on the transformed scale, and the confidence bands must reflect the statistical behavior of the mapped domain.

After computing the initial MOS values, the dataset must undergo observer screening to eliminate unreliable participants whose scoring patterns deviate significantly from the overall distribution. According to ITU-R BT.500-15~\cite{ITU-R-BT500}, a kurtosis-based post-screening method is recommended for DSIS, DSCQS, and other alternative methods (excluding SSCQE), especially in studies with relatively few observers (e.g., $N < 20$) or non-expert participants.

For each presentation defined by condition $j$, image or sequence $k$, and repetition $r$, the following statistics are computed: mean score, $\bar{u}_{jkr}$; standard deviation, $S_{jkr}$; and kurtosis coefficient, $\beta_{2,jkr}$, given by:

\begin{equation}
\beta_{2,jkr} = \frac{m_4}{(m_2)^2}, \quad \text{where } m_x = \frac{1}{N} \sum_{i=1}^{N} (u_{ijkr} - \bar{u}_{jkr})^x
\end{equation}

If $2 \leq \beta_{2,jkr} \leq 4$, the distribution is considered normal. Otherwise, it is treated as non-normal. For each observer $i$, two counters are defined: $P_i$ and $Q_i$, which count the number of times an observer's score is an outlier above or below the acceptable range, respectively.

The outlier detection criteria per presentation are as follows:

If the distribution is normal:
\[
\begin{cases}
    u_{ijkr} \geq \bar{u}_{jkr} + 2 S_{jkr} & \Rightarrow P_i = P_i + 1 \\
    u_{ijkr} \leq \bar{u}_{jkr} - 2 S_{jkr} & \Rightarrow Q_i = Q_i + 1
\end{cases}
\]
...
\[
\begin{cases}
    u_{ijkr} \geq \bar{u}_{jkr} + \sqrt{20} \, S_{jkr} & \Rightarrow P_i = P_i + 1 \\
    u_{ijkr} \leq \bar{u}_{jkr} - \sqrt{20} \, S_{jkr} & \Rightarrow Q_i = Q_i + 1
\end{cases}
\]

After processing all presentations, observer $i$ is rejected if:
\begin{equation*}
\frac{P_i + Q_i}{L} > 0.05 \quad \text{and} \quad \left| \frac{P_i - Q_i}{P_i + Q_i} \right| < 0.3
\end{equation*}
where $L$ is the total number of presentations.

This procedure ensures that only observers with balanced and statistically consistent scoring behavior are retained.
...
\begin{longtable}{
  >{\centering\arraybackslash}p{0.15\textwidth}  % Metric
  >{\centering\arraybackslash}p{0.10\textwidth}  % Category
  >{\centering\arraybackslash}p{0.05\textwidth}  % Year
  >{\centering\arraybackslash}p{0.55\textwidth}    % Tested Databases
}
    \caption{Summary of the 40 FR IQA metrics used}\label{tab:fr_iqa_metrics}\\
    \hline % chktex 44
    \textbf{Metric} & \textbf{Category} & \textbf{Year} & \textbf{Tested Databases} \\
    \hline % chktex 44
    ...
    MSE~\cite{wang2004image} & Error & 2004 & LIVE IQA, CSIQ, TID2008, TID2013, KADID-10k \\
    RMSE~\cite{wang2004image} & Error & 2004 & LIVE IQA, CSIQ, TID2008, TID2013, KADID-10k \\ 
    MAE~\cite{wang2004image} & Error & 2004 & LIVE IQA, CSIQ, TID2008, TID2013, KADID-10k \\
    PSNR~\cite{wang2004image} & Error & 2004 & LIVE IQA, CSIQ, TID2008, TID2013, KADID-10k \\
    PSNR-B~\cite{ma2011psnrb} & Error & 2011 & LIVE IQA, CSIQ, TID2013 \\
    SNR~\cite{wang2004image} & Error & 2004 & LIVE IQA, CSIQ, TID2008 \\
    SAM~\cite{yuhas1992sam} & Spectral & 1992 & AVIRIS, HYDICE, ROSIS, TID2013 \\
    ERGAS~\cite{wald2000ergas} & Spectral & 2000 & AVIRIS, ROSIS, TID2013,  KADID-10k \\
    $D_\lambda$~\cite{wald2000ergas} & Spectral & 2000 & AVIRIS, ROSIS, TID2013, KADID-10k \\
...
Error-based metrics quantify distortion by computing direct numerical differences between the reference and distorted images~\cite{wang2004image}. Let \( X = \{x_i\}_{i=1}^N \) and \( Y = \{y_i\}_{i=1}^N \) be two images with \( N \) pixels each. The pixel-wise error is defined as \( e_i = x_i - y_i \). These metrics are mathematically simple and computationally efficient but lack perceptual modeling.
...
The most basic error-based metric is the Mean Squared Error (MSE)~\cite{wang2004image}, which quantifies the average squared difference between corresponding pixels in the reference and distorted images:

\begin{equation}
\text{MSE} = \frac{1}{N} \sum_{i=1}^{N} (x_i - y_i)^2
\end{equation}

The Root Mean Squared Error (RMSE)~\cite{wang2004image} is a direct extension of the mean squared error, obtained by taking its square root:
...
The Mean Absolute Error (MAE)~\cite{wang2004image} measures the average absolute difference between the reference and distorted image pixels:

\begin{equation}
\text{MAE} = \frac{1}{N} \sum_{i=1}^{N} |x_i - y_i|
\end{equation}

Unlike MSE and RMSE, MAE penalizes all errors linearly, making it less sensitive to large deviations and outliers. This results in a more stable error measure in scenarios with isolated distortions or non-Gaussian noise.

The Peak Signal-to-Noise Ratio (PSNR)~\cite{wang2004image} expresses the fidelity between a reference and a distorted image using a logarithmic decibel (dB) scale. It is defined as:
...
The Blocking Effect Aware Peak Signal-to-Noise Ratio (PSNR-B)~\cite{ma2011psnrb, yim2011psnrb} was introduced to address one of the main weaknesses of PSNR, its insensitivity to compression artifacts such as blocking. PSNR-B extends the original formula by adding a blocking effect factor (BEF) to the denominator:

\begin{equation}
\text{PSNR-B} = 10 \cdot \log_{10} \left( \frac{MAX^2}{\text{MSE} + \text{BEF}} \right)
\end{equation}

The blocking effect factor penalizes the presence of visible discontinuities along block boundaries, which commonly result from transform-based compression schemes.

The Signal-to-Noise Ratio (SNR)~\cite{wang2004image} quantifies the proportion of signal power relative to noise power between a reference image and its distorted counterpart. It is computed as:
...
\subsubsection{Similarity-based metrics}
