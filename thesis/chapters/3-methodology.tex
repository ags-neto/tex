...
This section describes the experimental framework adopted to develop and evaluate the proposed image quality assessment pipeline. The methodology integrates both subjective and objective approaches, structured into successive stages. First, we introduce the datasets and the subjective quality assessment protocols used to collect and process mean opinion scores (MOS). Next, we present the objective evaluation pipeline, including full-reference (FR) metrics grouped by category and no-reference (NR) facial image quality assessment (FIQA) methods. We then describe the generation of pseudo-MOS through fusion of FR metrics, which provides weak supervision for subsequent regression models. Different regression families, including linear, kernel-based, and tree-ensemble methods, are compared as fusion engines. Building on this, we detail the training of a convolutional neural network (CNN) for NR-IQA under pseudo-MOS supervision. To ensure fairness and robustness, a debiasing procedure based on multi-way ANOVA is applied to subjective scores. Finally, we define the evaluation protocol, reporting correlations and error measures on both labeled and disjoint test sets. This layered design ensures consistency across subjective, objective, and learned components of the study.
